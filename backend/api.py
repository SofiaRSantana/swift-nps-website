...
CORS(app, origins=["https://swift-nps-website.vercel.app"])
...
if __name__ == "__main__":
    ...
    load_data()
    print("✓ Swift NPS Analytics rodando em http://localhost:5050")
...
DATA_DIR   = BASE_DIR.parent / "BasesDadosUtilizadas"
FILE_NPS   = DATA_DIR / "Dados para o Grillo.xlsx"
...
def load_data():
    if "data" in _cache:
        return _cache["data"]
    df = pd.read_excel(FILE_NPS)
    df_lojas = pd.read_excel(FILE_LOJAS)

    df["Mes Ano"]        = pd.to_datetime(df["Mes Ano"])
    df["Data Avaliação"] = pd.to_datetime(df["Data Avaliação"], errors="coerce")
    df_lojas["Mes Ano"]  = pd.to_datetime(df_lojas["Mes Ano"])
...
def nps_mensal():
    ...
    cols = ["ano_mes", "nps", "pct_promotor", "pct_neutro", "pct_detrator",
            "total", "promotor", "neutro", "detrator"]
    return jsonify(mensal[cols].rename(columns={"ano_mes": "label"}).to_dict(orient="records"))
...
def ranking_lojas():
    ...
    top5 = grp.nlargest(5, "nps")[["nome_curto", "nps"]].to_dict(orient="records")
    bot5 = grp.nsmallest(5, "nps")[["nome_curto", "nps"]].to_dict(orient="records")
...
@app.route("/api/temas")
def temas():
    # Resultados fixos da clusterização BERTopic (notebook clusterizacao_nps.ipynb)
    return jsonify([
        {"categoria": "Preços e Experiência Geral", "pct": 19.9, "count": 9958,  "nps_perfil": "90,6% promotores"},
        {"categoria": "Atendimento Positivo",        "pct": 15.4, "count": 7707,  "nps_perfil": "97,9% promotores"},
        {"categoria": "Qualidade dos Produtos",      "pct": 13.4, "count": 6679,  "nps_perfil": "56,9% promotores"},
...
def cobertura_comentarios():
    ...
    com_cnt = df[
        df["Comentario"].notna() &
        ~df["Comentario"].isin(["-", ""])
    ].groupby("classificacao")["qtd_clientes"].sum()
